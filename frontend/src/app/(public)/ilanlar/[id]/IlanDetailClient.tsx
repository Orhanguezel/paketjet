...
export default function IlanDetailClient({ilan}:{ilan:PublicIlan}) {
  ...
  useEffect(()=>{if(ilan.is_sample)return;let active=true;getListingCreditPrice().then(p=>{if(active)setPrice(p);}).catch(()=>{});getPaymentAvailability().then(p=>{if(active)setEnabled(p.enabled);}).catch(()=>{});return()=>{active=false;};},[attempt,ilan.is_sample]);
  useEffect(()=>{setAccess(null);if(!isAuthenticated||ilan.is_sample)return;let active=true;getListingAccess(ilan.id).then(a=>{if(active){setAccess(a);setError('');}}).catch(()=>{if(active)setError('Hesap durumu alınamadı. Yeniden deneyin.');});return()=>{active=false;};},[ilan.id,ilan.is_sample,isAuthenticated,attempt]);
  ...
  return <section className="site-container py-8 sm:py-10">
    ...
    <div className="grid items-start gap-8 lg:grid-cols-[1.65fr_1fr]"><div className="min-w-0">
      {ilan.is_sample&&<p className="mb-4 font-semibold text-brand">ÖRNEK İLAN — Tanıtım amaçlıdır</p>}
    ...
    </div><div>{ilan.is_sample?<aside className="rounded-2xl border border-border bg-brand-xlight p-6"><h2 className="text-xl font-semibold">Bu bir örnek ilandır</h2><p className="mt-3 leading-7 text-muted">Güzergâh, tarih ve araç bilgileri siteyi tanıtmak için hazırlanmıştır. Gerçek taşıma hizmeti sunulmaz; iletişim satın alınamaz ve ödeme yapılmaz.</p><Link href={ROUTES.ilanlar.list} className="mt-5 inline-flex min-h-11 items-center font-semibold text-brand">İlanlara dön →</Link></aside>:<><RevealAside contact={access?.contact??null} error={error} isAuthenticated={isAuthenticated} isActive={ilan.status==='active'} listingPrice={price} revealing={busy} state={access?.state} paymentsEnabled={enabled} onLogin={login} onPay={d=>purchase(d,true)} onReveal={d=>purchase(d,false)}/>{(error||price===null)&&<button onClick={()=>setAttempt(n=>n+1)} className="mt-3 min-h-11 text-brand">Bilgileri yeniden yükle</button>}</>}</div></div>
