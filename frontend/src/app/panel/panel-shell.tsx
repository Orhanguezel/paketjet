...
export default function PanelShell({children,logoUrl}:{children:React.ReactNode;logoUrl?:string}) {
  ...
  return <div className="min-h-screen bg-surface text-foreground md:grid md:grid-cols-[240px_1fr]">
    <aside className="bg-navy px-4 py-5 text-white md:min-h-screen"><div className="flex items-center justify-between"><Link href="/" className="flex items-center gap-3 px-3 py-2"><img src={logoUrl||"/uploads/media/logo/logo-transparent.png"} alt="PaketJet" width="40" height="40" className="rounded-lg"/><span className="text-xl font-bold">PaketJet</span></Link><button className="grid size-11 place-items-center md:hidden" aria-label="Panel menüsü" aria-expanded={open} aria-controls="panel-navigation" onClick={()=>setOpen(x=>!x)}><Menu size={22}/></button></div>
    ...
    <div className="min-w-0"><header className="flex min-h-18 items-center justify-between gap-4 border-b border-border-soft px-5 sm:px-8"><span className="font-medium">Hesabım</span><div className="flex min-w-0 items-center gap-4"><ThemeToggle/><Link href="/panel/profil" className="max-w-48 truncate text-sm">{user?.full_name||'Profilim'}</Link></div></header><main id="main-content" className="mx-auto max-w-6xl px-4 py-8 sm:px-8">{children}</main></div>
  </div>;
}
