...
import { ArrowRight, FileText, ShoppingBag, CalendarDays } from 'lucide-react';
...
export function RecentPurchases({rows,loading}:{rows:MyPurchase[]|null;loading:boolean}) {
 return <section className="account-card"><div className="account-card-heading"><h2>Son iletişim erişimlerim</h2><Link href={ROUTES.panel.satinAldiklarim}>Tümünü gör <ArrowRight size={15}/></Link></div>
  {loading?<p role="status" className="py-8 text-muted">İletişim erişimleri yükleniyor…</p>:rows===null?<p className="py-8 text-muted">Satın alma bilgisi alınamadı.</p>:rows.length?<ul className="divide-y divide-border-soft">{rows.slice(0,4).map(row=><li key={row.id}><Link href={ROUTES.ilanlar.detail(row.ilan_id)} className="flex items-center justify-between gap-3 py-4 font-medium"><span>{row.from_city} → {row.to_city}</span><ArrowRight size={17}/></Link></li>)}</ul>:<div className="account-empty"><ShoppingBag size={32}/><h3>Henüz iletişim erişimin yok.</h3><p>Taşıyıcılarla iletişime geçmek için ilanları keşfet.</p><Link href={ROUTES.ilanlar.list}>İlanları keşfet <ArrowRight size={16}/></Link></div>}
 </section>;
}
