...
export default function IlanCard({ilan}: {ilan:Ilan;listingCreditPrice?:number|null}) {
  return <article className="listing-card grid gap-4 rounded-2xl border border-border-soft bg-surface p-6 md:grid-cols-[1.1fr_1.1fr_0.7fr_1.2fr_auto] md:items-center">
    <div className="min-w-0"><p className="mb-1 text-sm text-muted">{ilan.is_sample?'ÖRNEK İLAN':'Güzergâh'}</p><h3 className="flex flex-wrap items-center gap-2 text-lg font-semibold"><span>{ilan.from_city}</span><svg aria-hidden="true" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M4 12h16m-6-6 6 6-6 6"/></svg><span>{ilan.to_city}</span></h3>{(ilan.from_district||ilan.to_district)&&<p className="mt-1 text-sm text-muted">{ilan.from_district} / {ilan.to_district}</p>}</div>
