...
export default function Footer({logoUrl,logoAlt,contact,socials}:FooterProps){
  return <footer className="site-footer"><div className="site-container">
    <div className="grid grid-cols-2 gap-x-8 gap-y-9 lg:grid-cols-[1.5fr_1fr_1fr_1fr]">
      <div className="col-span-2 lg:col-span-1"><Link href={ROUTES.home} aria-label="PaketJet ana sayfa" className="footer-brand">{logoUrl&&<span className="footer-logo"><img src={logoUrl} alt={logoAlt??'PaketJet'} width={60} height={60}/></span>}<span className="footer-wordmark">PaketJet</span></Link><p className="footer-muted mt-4 max-w-64 text-sm leading-6">Taşıyıcıyla doğrudan iletişim.</p></div>
      <nav aria-label="Ürün bağlantıları"><h2 className="mb-5 text-sm font-semibold">Keşfet</h2><div className="footer-muted flex flex-col gap-2 text-sm"><Link href={ROUTES.ilanlar.list}>İlanlar</Link><Link href="/#nasil-calisir">Nasıl çalışır</Link><Link href={ROUTES.ilanVer}>Ücretsiz ilan ver</Link><Link href="/blog">Kargo rehberleri</Link></div></nav>
