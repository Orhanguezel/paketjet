import {and,or,eq,like,sql,type SQL} from 'drizzle-orm';
...
/** Province alternatives must not match a similarly named district or street. */
export function provinceFilter(side:'from'|'to',province:string):SQL{
 return or(eq(ilanlar[`${side}_city`],province),sql`JSON_UNQUOTE(JSON_EXTRACT(${ilanlar[`${side}_location`]}, '$.city')) = ${province}`)!;
}
