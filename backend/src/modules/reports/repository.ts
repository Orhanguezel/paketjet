...
export async function repoGetCommissionReport(range: ReportsDateRange) {
  const rows = await db.execute(sql`
    SELECT
      b.id AS booking_id,
      b.carrier_id,
      ...
      b.commission_rate,
      b.commission_amount,
      ROUND(b.total_price - b.commission_amount, 2) AS carrier_payout,
      b.status,
      b.created_at
    FROM bookings b
    LEFT JOIN users u ON u.id = b.carrier_id
    WHERE b.status = 'delivered'
      AND CAST(b.commission_amount AS DECIMAL(14,2)) > 0
      ${range.from ? sql`AND b.created_at >= ${range.from}` : sql``}
      ${range.to   ? sql`AND b.created_at <= ${range.to}`   : sql``}
  ...
  const summaryRows = await db.execute(sql`
    SELECT
      ...
      COALESCE(SUM(b.total_price - b.commission_amount), 0) AS total_carrier_payouts
    FROM bookings b
    WHERE b.status = 'delivered'
      AND CAST(b.commission_amount AS DECIMAL(14,2)) > 0
      ${range.from ? sql`AND b.created_at >= ${range.from}` : sql``}
  ...
  return { items: rows, summary: (summaryRows as unknown[])[0] ?? {} };
}
